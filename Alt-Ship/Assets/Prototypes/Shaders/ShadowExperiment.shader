Shader "EE/ShadowExperiment"
{
    Properties
    {
        ...
        _SpecularColor ("Specular Color", Color) = (1, 1, 1, 1.0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings IN) : SV_Target
            {
                ...
                float3 viewDir = normalize(_WorldSpaceCameraPos - IN.posWS); // Direction from surface point to camera
                float3 halfDir = normalize(_MainLightPosition.xyz + viewDir); // Half vector between light direction and view direction
                float NdotH = saturate(dot(IN.normalWS, halfDir));
                
                // Calculate diffuse and specular components
                float3 diffuse = _MainLightColor.rgb * _DiffuseColor.rgb * NdotL;
                float specular = _SpecularColor.rgb * pow(NdotH, _Shininess);
                
                // Combine lighting with shadows
                float3 col = (diffuse + specular) * shadowValue;
